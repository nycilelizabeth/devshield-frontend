    ...
    <aside className="w-60 min-h-screen bg-slate-900 flex flex-col flex-shrink-0">
              ...
              <NavLink key={item.to} to={item.to}
                className={({ isActive }) =>
                  `flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm font-medium transition-all mb-0.5 border-l-2 ${
                    isActive ? 'bg-blue-900/40 text-white border-blue-500' : 'text-slate-400 border-transparent hover:bg-slate-800 hover:text-slate-200'
                  }`}>
      ...
      <div className="px-4 py-4 border-t border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleLogout} className="text-slate-600 hover:text-red-400 transition-colors text-sm">⏻</button>
        </div>
      </div>
    </aside>
  )
