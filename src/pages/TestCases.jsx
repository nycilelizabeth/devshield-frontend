// ============================================
// pages/TestCases.jsx — Test Case Manager
// ============================================
...
export default function TestCases() {
  ...
  const [form, setForm]         = useState({
    ...
    priority:'Medium', module:'General', project:'General'
  })

  const load = async () => {
    try {
      const res = await testAPI.getAll()
      setTests(res.data.testCases || [])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ title:'', description:'', steps:'', expectedResult:'', priority:'Medium', module:'General', project:'General' })
      setShowForm(false)
      notify('✅ Test case created!')
      load()
    } catch(e){ notify('❌ Failed to create test case') }
    finally { setSaving(false) }
  }

  const runTest = async (id, result) => {
    try {
      await testAPI.update(id, { status: result, actualResult: result === 'Pass' ? 'Test passed successfully' : 'Test failed — check steps' })
      notify(`${result === 'Pass' ? '✅' : '❌'} Test marked as ${result}`)
      load()
  ...
  const statusBadge = {
    ...
    'Not Run':'bg-slate-50 text-slate-500 border-slate-200',
  }
  const priBadge = {
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
        ...
        <div className="flex gap-6 mt-3">
          {[['✅','Passed',stats.pass,'text-green-600'],['❌','Failed',stats.fail,'text-red-600'],
            ['⬜','Not Run',stats.notRun,'text-slate-500'],['📋','Total',stats.total,'text-blue-600']].map(([ico,lbl,val,col])=>(
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input className="w-full px-3.5 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="e.g. Valid login with correct credentials" value={form.title}
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <input className="w-full px-3.5 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="e.g. Auth, Vault, API" value={form.module}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <textarea className="w-full px-3.5 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none h-24"
                  placeholder="User should be redirected to /dashboard with their name shown in the top bar" value={form.expectedResult}
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-slate-400 text-sm mt-1">
              {tests.length === 0 ? 'Add your first test case using the button above' : 'Try a different filter'}
            </p>
          </div>
        ) : filtered.map((t,i) => (
          <div key={t._id} className={`px-5 py-4 flex items-start gap-4 ${i!==filtered.length-1?'border-b border-slate-100':''} hover:bg-slate-50/50 transition-colors`}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                ...
                {t.module  && <span className="text-xs font-semibold px-2 py-0.5 rounded border bg-blue-50 text-blue-600 border-blue-200">{t.module}</span>}
                {t.project && <span className="text-xs text-slate-400 font-mono">{t.project}</span>}
              </div>
              <p className="font-semibold text-slate-800 text-sm mb-1">{t.title}</p>
              {t.expectedResult && (
                <p className="text-xs text-slate-500">
                  <span className="font-semibold text-slate-600">Expected: </span>{t.expectedResult}
                </p>
              )}
              {t.steps && (
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <button onClick={()=>runTest(t._id,'Pass')}
              ...
              <button onClick={()=>runTest(t._id,'Fail')}
                className="text-xs bg-red-50 text-red-700 border border-red-200 px-2.5 py-1.5 rounded-lg font-semibold hover:bg-red-100 transition">
                ✗ Fail
              </button>
